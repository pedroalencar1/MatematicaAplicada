...
#' acordo com o modelo logístico (modelo de Verhulst):
#'
#' dN/dt = rN(1 - N/K), com:
...
#' 4) Discutir o que acontece se alterarmos r e/ou K.

#%% 3.1 Resolução da Atividade ------
ode_fun_eutrofizacao <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    dN <- r * N * (1 - N / K)
    list(c(dN))
  })
...
state_eutrofizacao <- c(N = 50)  # biomassa inicial em g/m²
parameters_eutrofizacao <- c(r = 0.5, K = 1000) # r em mês⁻¹ e K em g/m²
times_eutrofizacao <- seq(0, 12, by = 0.1) # tempo em meses

out_eutrofizacao <- ode(y = state_eutrofizacao, times = times_eutrofizacao,
  func = ode_fun_eutrofizacao, parms = parameters_eutrofizacao) |>
  as.data.frame()

head(out_eutrofizacao)

#%% 2) Biomassa após 6 meses
biomassa_6_meses <- out_eutrofizacao[out_eutrofizacao$time == 6, "N"]
biomassa_6_meses

#%% 3) Plotando resultados ------
ggplot(as.data.frame(out_eutrofizacao), aes(x = time, y = N)) +
  geom_line(color = "blue", size = 1.5) +
  labs(x = "Tempo (meses)", y = "Biomassa de Macrófitas (g/m²)") +
...
#%% 4) Discussão
#' Se K aumentar (mais nutrientes), a biomassa máxima suportada pelo lago
#' aumentará, permitindo um crescimento maior da biomassa de macrófitas.
#' Se r aumentar, a taxa de crescimento inicial será mais rápida, levando a
#' um aumento mais rápido da biomassa até atingir a capacidade de suporte K.

#%% 4. Atividade

#' Um solo possui as seguintes características:
#' Condutividade saturada Ks = 0.01 m/h
#' Potencial de sucção na frente de umidade ψf = 0.1 m
#' Umidade volumétrica saturada θs = 0.45
#' Umidade volumétrica inicial θi = 0.15
...
#%% 4.1 Resolução da Atividade ------

#1)	resolvendo numericamente a equação
ode_fun_infiltracao <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {

    # Avoiding "no visible binding" notes
    # F <- state[["F"]]
    # Ks <- parameters[["Ks"]]
    # psi_f <- parameters[["psi_f"]]
    # theta_s <- parameters[["theta_s"]]
    # theta_i <- parameters[["theta_i"]]

    dF <- Ks * (1 + psi_f * (theta_s - theta_i) / F)
    list(c(dF))
  })
}

state_infiltracao <- c(F = 0.0001)  # infiltração inicial em m
parameters_infiltracao <- c(Ks = 0.4, psi_f = 0.1,
							theta_s = 0.45, theta_i = 0.05)
times_infiltracao <- seq(0, 10, by = 0.1) # tempo em horas

out_infiltracao <- ode(y = state_infiltracao, times = times_infiltracao,
  func = ode_fun_infiltracao, parms = parameters_infiltracao) |>
  as.data.frame()
head(out_infiltracao)

# 1.1) plotando resultados

ggplot(as.data.frame(out_infiltracao), aes(x = time, y = F)) +
  geom_line(color = "blue", size = 1.5) +
  labs(x = "Tempo (horas)", y = "Infiltração Acumulada F(t) (m)") +
  scale_x_continuous(breaks = seq(0, 10, by = 1)) +
#   scale_y_continuous(breaks = seq(0, 0.2, by = 0.02), limits = c(0, 0.2)) +
  theme_minimal() +
  theme(text = element_text(size = 30))

# 2) Estimando graficamente a taxa de infiltração instantânea
#' A taxa de infiltração instantânea f(t) = dF/dt pode ser estimada
#' graficamente pela inclinação da curva de infiltração acumulada F(t).
#' Note que a taxa de infiltração é maior no início e diminui com o tempo,
#' aproximando-se de um valor constante à medida que o solo se satura.

# Aproximação da derivada
f_t <- diff(out_infiltracao$F) / diff(out_infiltracao$time)  # ΔF/Δt
t_mid <- out_infiltracao$time[-1]  # tempos correspondentes às derivadas

ggplot() +
  geom_line(aes(x = t_mid, y = f_t), color = "red", size = 1.5) +
  labs(x = "Tempo (h)", y = "Taxa de infiltração f(t) [m/h]") +
  scale_x_continuous(breaks = seq(0, 10, by = 1)) +
...
# 3) Discussão
#' A taxa de infiltração f(t) diminui com o tempo, pois à medida que o solo
#' se satura, a capacidade de absorção de água diminui.
#' Solos mais arenosos tendem a ter uma taxa de infiltração inicial mais alta
#' devido à maior porosidade, enquanto solos mais argilosos têm uma taxa
#' inicial mais baixa devido à menor porosidade e maior retenção de água.
