...
cat(
  "Solução do sistema de equações não lineares é: x =",
  solution$x[1],
  ", y =",
  solution$x[2],
  "\n"
...
fig <- plot_ly() %>%
  ...
  layout(
    title = "Sistema de Equações Não Lineares",
    scene = list(
      xaxis = list(title = "X"),
      yaxis = list(title = "Y"),
      zaxis = list(title = "Z")
...
cat(
  "A infiltração acumulada F para t = 2h usando Newton-Raphson é:",
  F_solution_nr,
  "cm\n"
)
...
cat(
  "A infiltração acumulada F para t = 2h usando uniroot é:",
  F_solution,
  "cm\n"
)
...
cat(
  "A infiltração acumulada F para t = 2h usando nleqslv é:",
  F_solution_nleqslv,
  "cm\n"
)
...
ggplot() +
  ...
  geom_text(
    data = extrema,
    aes(x = x, y = f_x, label = round(f_x, 2)),
    vjust = -1,
    color = "black",
    size = 6
  ) +
  labs(title = "Otimização de Função Não Linear", x = "x", y = "f(x)") +
  theme_minimal() +
  theme(text = element_text(size = 30))
...
fig <- plot_ly(x = ~x, y = ~y, z = ~z, type = "surface") %>%
  layout(
    title = "Função de Produção com Restrição Orçamentária",
    scene = list(
      xaxis = list(title = "Fertilizante A (kg)"),
